...
test.describe('仮認証フロー', () => {
  test('未ログイン時は管理画面にアクセスできずサインイン画面へリダイレクト', async ({ page }) => {
    await page.goto('/admin/spots');
    await expect(page).toHaveURL(/\/auth\/signin/);
  });

  test('admin@example.comでサインイン→管理画面→サインアウト→再度アクセスでリダイレクト', async ({ page }) => {
  ...
  test('サインイン済みでサインイン画面にアクセス→自動で管理画面へ遷移', async ({ page }) => {
    await page.goto('/auth/signin');
    await page.getByLabel('メールアドレス').fill('admin@example.com');
    await page.getByRole('button', { name: 'サインイン（メール）' }).click();
    await expect(page).toHaveURL(/\/admin\/spots/);
