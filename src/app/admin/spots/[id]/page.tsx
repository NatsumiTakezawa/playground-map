...
export default function AdminSpotEditPage() {
  ...
  useEffect(() => {
    async function fetchSpot() {
      setLoading(true);
      const { data, error } = await supabase.from("onsen").select("*").eq("id", id).single();
      if (error || !data) {
  ...
  return (
    <main className="min-h-screen bg-primary-50 p-8">
      ...
      <form onSubmit={handleSave} className="space-y-6 max-w-xl bg-white rounded shadow p-8">
        <div>
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full border rounded p-2"
              value={form.geo_lat ?? ""}
              onChange={e => setForm(f => ({ ...f, geo_lat: parseFloat(e.target.value) }))}
              required
            />
          </div>
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full border rounded p-2"
              value={form.geo_lng ?? ""}
              onChange={e => setForm(f => ({ ...f, geo_lng: parseFloat(e.target.value) }))}
              required
            />
          </div>
        </div>
        ...
        {/* MapPicker（ダミー） */}
        <div>
          <label className="block font-bold mb-1">地図で位置を選択（今後実装）</label>
          <div className="w-full h-48 bg-gray-100 rounded flex items-center justify-center text-gray-400">MapPicker（ダミー）</div>
        </div>
        <div className="flex gap-4 mt-6">
          <button type="submit" className="bg-primary-500 text-white px-6 py-2 rounded font-bold hover:bg-primary-700 transition">保存</button>
          <button type="button" className="bg-gray-100 text-gray-700 px-6 py-2 rounded font-bold border hover:bg-red-100" onClick={() => router.push("/admin/spots")}>キャンセル</button>
        </div>
      </form>
    </main>
  );
